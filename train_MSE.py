...
for epoch in range(opt.niter):
    ...
    while i < len(dataloader):
        ...
        while j < Diters and i < len(dataloader):
            ...
            errD_real_vec = netD(Variable(torch.cat((input, perceptualRes_cpu), 1)))
            ...
            errD_fake_vec = netD(torch.cat((fake / scaler + Variable(perceptualRes_cpu), Variable(perceptualRes_cpu)), 1))
            ...
            dist = L2_dist(Variable(input).view(batch_size, -1),
                           fake.div(scaler).add(Variable(perceptualRes_cpu)).view(batch_size, -1))
        ...
        if i < len(dataloader):
            ...
            if flag:  # fix samples
                ...
                fixed_PER.resize_as_(real_cpu).copy_(perceptualRes_cpu)
                viz.images(
                    fixed_PER.mul(0.5).add(0.5).cpu().numpy(),
            ...
            if gen_iterations < opt.baseGeni:
                ...
                errG = netD(torch.cat((fake / scaler + Variable(perceptualRes_cpu), Variable(perceptualRes_cpu)), 1)).mean(0).view(1)
