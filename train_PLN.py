...
#         residual = out
#
#         noise = torch.FloatTensor(out.size()[0], 100, 1, 1)
...
for epoch in range(opt.niter):
    ...
    while i < len(dataloader):
        ...
        while j < Diters and i < len(dataloader):
            ...
            input.resize_as_(real_cpu).copy_(real_cpu - perceptualRes_cpu)
            zasshu.resize_as_(fake_cpu).copy_(fake_cpu)

            inputv = Variable(torch.cat((input, perceptualRes_cpu), 1))
            ...
            fake = Variable(netG(inputv).data - perceptualRes_cpu)
            errD_fake_vec = netD(torch.cat((fake, Variable(perceptualRes_cpu)), 1))
        ...
        if i < len(dataloader):
            ...
            if gen_iterations < opt.baseGeni:
                ...
                errG = opt.ganW * netD(
                    torch.cat((fake - Variable(perceptualRes_cpu), Variable(perceptualRes_cpu)), 1)).mean(0).view(1)
